# Munges files using the Genomic SEM package
 ...
 setwd("~/Library/CloudStorage/Box-Box/GENIE_GSEM/")
...
file.rename(
  ...
  to = file.path(folderpaths$data.gwas_sumstats.munged.hm3_munged_info_0_8_maf_0_1, basename(list.files(pattern = ".gz", full.names = TRUE)))
)

# Move log files to the munged folder
file.rename(
  from = list.files(pattern = ".log", full.names = TRUE),
  to = file.path(folderpaths$data.gwas_sumstats.munged.hm3_munged_info_0_8_maf_0_1, basename(list.files(pattern = ".log", full.names = TRUE)))
)
