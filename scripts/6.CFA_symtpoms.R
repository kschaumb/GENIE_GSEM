...
# ============================================================
# Outputs -> gsem/gsem_output/symptoms_cfa/
# Usage   -> source("scripts/cfa_dwls_pipeline_symptoms.R")
# Notes   -> - DWLS only
#           - Constrained model: Development ~~ 0*Internal and Anthro ~~ 0*Internal
#             (via shared label + equality constraint)
#           - Sensitivity: optional residual floors (none applied by default here)
...
out_dir     <- file.path(base_path, "gsem/gsem_output/symptoms_cfa")
...
# Sensitivity floors to apply to selected residual variances (set to nonzero if desired)
residual_floors <- c(0.00, 0.01)

# Indicators to guard with residual floors (none by default for symptoms)
guard_indicators <- character(0)
...
log_msg("Starting DWLS CFA pipeline (SYMPTOMS; consolidated outputs)")
...
rename_INT <- function(x) gsub("\\bINT\\b", "Internal", x)
...
# Robust fit row builder: also pulls TLI from lavaan if modelfit lacks it
row_from_fit <- function(nm, f) {
  mf <- if ("modelfit" %in% names(f)) f$modelfit else NULL
  grab_num <- function(x) if (!is.null(x) && length(x) == 1) as.numeric(x) else NA_real_
  
  chisq <- if (!is.null(mf)) grab_num(mf$chisq) else NA_real_
  dfv   <- if (!is.null(mf) && !is.null(mf$df) && length(mf$df)==1) as.integer(mf$df) else NA_integer_
  pval  <- if (!is.null(mf)) grab_num(mf$p_chisq) else NA_real_
  aic   <- if (!is.null(mf)) grab_num(mf$AIC) else NA_real_
  cfi   <- if (!is.null(mf)) grab_num(if (!is.null(mf$CFI)) mf$CFI else mf$cfi) else NA_real_
  tli   <- if (!is.null(mf)) grab_num(if (!is.null(mf$TLI)) mf$TLI else mf$tli) else NA_real_
  srmr  <- if (!is.null(mf)) grab_num(mf$SRMR) else NA_real_
  
  if (("lavaan_output" %in% names(f)) && inherits(f$lavaan_output, "lavaan")) {
    fm <- try(lavaan::fitMeasures(f$lavaan_output, c("chisq","df","pvalue","aic","cfi","tli","srmr")), silent = TRUE)
    if (!inherits(fm, "try-error")) {
      if (is.na(chisq)) chisq <- grab_num(fm[["chisq"]])
      if (is.na(dfv))   dfv   <- if (!is.na(fm[["df"]])) as.integer(fm[["df"]]) else NA_integer_
      if (is.na(pval))  pval  <- grab_num(fm[["pvalue"]])
      if (is.na(aic))   aic   <- grab_num(fm[["aic"]])
      if (is.na(cfi))   cfi   <- grab_num(fm[["cfi"]])
      if (is.na(tli))   tli   <- grab_num(fm[["tli"]])
      if (is.na(srmr))  srmr  <- grab_num(fm[["srmr"]])
    }
  }
  
  data.frame(Model = nm, Chisq = chisq, df = dfv, p_chisq = pval,
             AIC = aic, CFI = cfi, TLI = tli, SRMR = srmr, Wald_p = NA_real_)
}
...
diag_one_model <- function(model_name, fit_obj, obs_names,
  ...
  neg_unstd <- subset(Th, flag_resid_neg_unstd)[, c("lhs","est"), drop=FALSE]    |> dplyr::rename(value = est);  if (nrow(neg_unstd)) neg_unstd$issue <- "neg_resid_unstd"
  neg_std   <- subset(Th, flag_resid_neg_std)[, c("lhs","std_resid"), drop=FALSE] |> dplyr::rename(value = std_resid); if (nrow(neg_std))   neg_std$issue   <- "neg_resid_std"
  gt1 <- subset(L, flag_loading_gt1)[, c("lhs","rhs","std_loading"), drop=FALSE] |> dplyr::rename(value = std_loading); if (nrow(gt1)) gt1$issue <- "loading_gt_1"
  hic <- subset(R, flag_hi_corr)[, c("lhs","rhs","std_corr"), drop=FALSE] |> dplyr::rename(value = std_corr); if (nrow(hic)) hic$issue <- "high_factor_corr"
  
  issues <- dplyr::bind_rows(neg_unstd, neg_std, gt1, hic); if (nrow(issues)) issues$Model <- model_name
...
# --------------------
# 4) Model Templates (SYMPTOMS)
# --------------------
# Original: all latent correlations freely estimated
model_original <- "
Anthropometric  =~ BMI + BMI_Childhood + BFP + MDD_WtGain_Case + FFM + MDD_WtLoss_Case + AN + BEBROAD
Development =~ Age_Menarche + BEBROAD + BMI_Childhood
...
# Constrained: Internalizing correlations with Anthropometric & Development fixed to 0, BEBROAD + Childhood BMI specific to development
model_constrained <- "
Anthropometric  =~ BMI + BFP + MDD_WtGain_Case + FFM + MDD_WtLoss_Case + AN
Development =~ Age_Menarche + BEBROAD + BMI_Childhood
Internalizing     =~ Dep_Sx + PTSD_PCS + Neurotic + ANX_FS + OCSymptoms + BEBROAD + AN
...
append_resid_floors <- function(model_txt, floor_value, indicators) {
  if (isTRUE(all.equal(floor_value, 0)) || !length(indicators)) return(model_txt)
...
for (f in residual_floors) {
  ...
  log_msg("Fitting original (DWLS), floor=", f);  fit_o <- fit_dwls(m_orig_f)
  log_msg("Fitting constrained (DWLS), floor=", f); fit_c <- fit_dwls(m_con_f)
  
  name_o <- paste0("orig_DWLS_", suffix)
  name_c <- paste0("constrained_DWLS_", suffix)
  fits[[name_o]] <- fit_o;  fits[[name_c]] <- fit_c
...
if (nm_base %in% names(fits) && "lavaan_output" %in% names(fits[[nm_base]])) {
  wobj <- try(lavaan::lavTestWald(fits[[nm_base]]$lavaan_output, "Development~~Internal == 0"), silent = TRUE)
...
consolidated <- list(
  ...
  wald = list(
    contrast = "Development~~Internal == 0 (unconstrained, base floor)",
    p_value  = wald_p,
  ...
  if (ok) {
    ...
    if (is.data.frame(consolidated$tables$issues) && !"message" %in% names(consolidated$tables$issues)) {
      ...
      openxlsx::writeData(wb, "issues",
                          if (is.data.frame(consolidated$tables$issues)) consolidated$tables$issues
                          else data.frame(message = "No flagged issues under current thresholds.")
      )
    }
    ...
    prov_txt <- paste(
      ...
      "",
      sprintf("Thresholds -> THRESH_LOAD: %s | THRESH_CORR: %s", THRESH_LOAD, THRESH_CORR),
