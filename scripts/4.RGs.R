#!/usr/bin/env Rscript
# ============================================================
# master_rg_matrices.R
# - One recoder for ALL runs
# - Keeps all file paths / outputs unchanged
#   * gsem/gsem_output/RGs/Genetic_Correlation_Matrix_ALL.png
#   * gsem/gsem_output/Genetic_Correlation_Matrix_Symptoms.png
#   * gsem/gsem_output/Genetic_Correlation_Matrix_Disorders.png
# Usage:
#   Rscript scripts/master_rg_matrices.R --which all,symptoms,disorders
#   Rscript scripts/master_rg_matrices.R --which symptoms
# ============================================================

suppressPackageStartupMessages({
  library(reshape2)
  library(ggplot2)
  library(dplyr)
  library(tibble)
})

# -----------------------
# Setup (unchanged)
# -----------------------
source("scripts/0.Packages.R")
load("gsem/gsem_settings/folderpaths.RData")
load("gsem/gsem_settings/sumstats_metadata.RData")
load("gsem/gsem_settings/variant_paths.RData")

# -----------------------
# CLI: --which all,symptoms,disorders
# -----------------------
args <- commandArgs(trailingOnly = TRUE)
which_to_run <- c("all","symptoms","disorders")
if (length(args)) {
  for (i in seq_along(args)) {
    if (identical(args[[i]], "--which") && i < length(args)) {
      which_to_run <- strsplit(args[[i+1]], ",", fixed = TRUE)[[1]]
    }
  }
}
which_to_run <- intersect(which_to_run, c("all","symptoms","disorders"))
if (length(which_to_run) == 0) which_to_run <- c("all","symptoms","disorders")

stamp <- function(...) cat(format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "-", ..., "\n")

# -----------------------
# Helper: read LDSC (with safe path fallback)
# -----------------------
read_ldsc_hm3 <- function(codes, primary_dir) {
  fname <- paste0("mvLD_", paste(codes, collapse = "_"), ".Rds")
  candidate1 <- file.path(primary_dir, fname)       # preferred
  candidate2 <- file.path("gsem", "mvld", fname)    # fallback
  
  chosen <- if (file.exists(candidate1)) candidate1 else candidate2
  if (!file.exists(chosen)) stop("❌ mvLD file not found: ", candidate1, " (or fallback ", candidate2, ")")
  
  ldsc <- readRDS(chosen)
  if (!is.null(ldsc$hm3)) ldsc <- ldsc$hm3
  if (is.null(rownames(ldsc$S))) rownames(ldsc$S) <- colnames(ldsc$S)
  ldsc$S <- (ldsc$S + t(ldsc$S)) / 2
  ldsc
}

# -----------------------
# Single, shared recoder
# -----------------------
clean_recode <- function(trait_order) {
  trait_order |>
    gsub("_Eur", "", x = _) |>
    gsub("_noAN", "", x = _) |>
    gsub("_StageMidAdol", " Stage", x = _) |>
    gsub("_", " ", x = _) %>%
    recode(
      # disorders / traits
      "AN" = "Anorexia Nervosa",
      "ANX" = "Anxiety Disorders",
      "BE noAN" = "Binge Eating (not AN ascertained)",
      "BMI Childhood" = "Childhood BMI",
      "BFP" = "Body Fat Percentage",
      "FFM" = "Fat-Free Mass",
      "PA" = "Physical Activity",
      "BE" = "Binge Eating",
      # symptoms / factors
      "ANR" = "AN [Restriction]",
      "ANX FS" = "Anxiety Factor Score",
      "OCSymptoms" = "OC Symptoms",
      "Depressive Sx" = "Depressive Symptoms",
      "MDD WtGain Case" = "MDD Weight Gain",
      "MDD WtLoss Case" = "MDD Weight Loss",
      "BEBROAD" = "Binge Eating",
      "Neurotic" = "Neuroticism", 
      "PTSD PCS" = "PTSD Symptoms",
      "Dep Sx" = "Depressive Symptoms"
    )
}

# -----------------------
# Core plotting function
# -----------------------
plot_rg_matrix <- function(ldsc, title_text, out_path) {
  trait_names <- colnames(ldsc$S)
  
  gen_cor <- cov2cor(ldsc$S)
  gen_cor[gen_cor > 1]  <- 1
  gen_cor[gen_cor < -1] <- -1
  gen_cor <- round(gen_cor, 2)
  rownames(gen_cor) <- trait_names
  colnames(gen_cor) <- trait_names
  
  # h2: prefer ldsc$h2, else diag(S)
  h2_vec <- if ("h2" %in% names(ldsc)) {
    hv <- as.numeric(ldsc$h2); names(hv) <- trait_names; hv
  } else {
    message("NOTE: ldsc$h2 not found; using diag(ldsc$S) as an approximation.")
    setNames(diag(ldsc$S), trait_names)
  }
  h2_vec <- pmin(pmax(h2_vec, 0), 1)
  h2_df  <- data.frame(Trait = trait_names, h2 = round(h2_vec, 2))
  
  hc <- hclust(as.dist(1 - abs(gen_cor)))
  trait_order <- rownames(gen_cor)[hc$order]
  
  cor_df <- reshape2::melt(gen_cor, varnames = c("Trait1","Trait2"), value.name = "rG")
  cor_df$Trait1 <- factor(cor_df$Trait1, levels = trait_order)
  cor_df$Trait2 <- factor(cor_df$Trait2, levels = trait_order)
  cor_df <- subset(cor_df, as.numeric(Trait1) <= as.numeric(Trait2))
  
  clean_names <- clean_recode(trait_order)
  names(clean_names) <- trait_order
  
  cor_df$Trait1 <- factor(cor_df$Trait1, levels = trait_order, labels = clean_names)
  cor_df$Trait2 <- factor(cor_df$Trait2, levels = trait_order, labels = clean_names)
  h2_df$Trait   <- factor(h2_df$Trait,   levels = trait_order, labels = clean_names)
  
  p <- ggplot() +
    geom_tile(data = subset(cor_df, Trait1 != Trait2),
              aes(x = Trait1, y = Trait2, fill = rG), color = "white") +
    geom_tile(data = subset(cor_df, Trait1 == Trait2),
              aes(x = Trait1, y = Trait2), fill = "white", color = "white") +
    geom_text(
      data = subset(cor_df, Trait1 != Trait2) |>
        mutate(label_fmt = sub("(?<=-|^)0\\.", ".", sprintf("%.2f", rG), perl = TRUE)),
      aes(x = Trait1, y = Trait2, label = label_fmt),
      size = 5
    ) +
    geom_text(
      data = h2_df,
      aes(x = Trait, y = Trait, label = sprintf("%.2f", h2)),
      size = 5, fontface = "bold", color = "#1A4F66"
    ) +
    scale_fill_gradient2(low = "#1A4F66", mid = "white", high = "#EF6C45",
                         midpoint = 0, limits = c(-1, 1),
                         name = expression(paste("Genetic ", italic("r")))) +
    scale_x_discrete(expand = c(0, 0)) +
    scale_y_discrete(limits = rev(levels(cor_df$Trait2)), expand = c(0, 0)) +
    coord_equal(clip = "off") +
    theme_minimal() +
    labs(title = title_text, x = "Trait", y = "Trait") +
    embarktools::embark_theme_a +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1),
      legend.position = "none",
      axis.title.x = element_blank(),
      axis.title.y = element_blank()
    )
  
  dir.create(dirname(out_path), recursive = TRUE, showWarnings = FALSE)
  ggsave(out_path, plot = p, width = 10, height = 10, units = "in")
  
  print(h2_df[order(as.numeric(h2_df$Trait)), ], row.names = FALSE)
}

# -----------------------
# Runs
# -----------------------
run_all <- function() {
  stamp("Running: ALL")
  codes <- (sumstats %>% filter(Symptom_RGs == 1 | Disorder_RGs == 1))$code
  ldsc <- read_ldsc_hm3(codes, folderpaths$gsem.mvLD)
  plot_rg_matrix(ldsc, "(a) Genetic Correlation Matrix - Key Traits and Disorders",
                 "gsem/gsem_output/RGs/Genetic_Correlation_Matrix_ALL.png")
}

run_symptoms <- function() {
  stamp("Running: SYMPTOMS")
  codes <- (sumstats %>% filter(Symptom_RGs == 1))$code
  ldsc <- read_ldsc_hm3(codes, folderpaths$gsem.mvLD)
  plot_rg_matrix(ldsc, "(b) Genetic Correlation Matrix - Key Traits and Symptoms",
                 "gsem/gsem_output/RGs/Genetic_Correlation_Matrix_Symptoms.png")
}

run_disorders <- function() {
  stamp("Running: DISORDERS")
  codes <- (sumstats %>% filter(Disorder_RGs == 1))$code
  ldsc <- read_ldsc_hm3(codes, folderpaths$gsem.mvLD)
  plot_rg_matrix(ldsc, "(a) Genetic Correlation Matrix - Key Traits and Disorders",
                 "gsem/gsem_output/RGs/Genetic_Correlation_Matrix_Disorders.png")
}

# -----------------------
# Execute
# -----------------------
if ("all" %in% which_to_run)       run_all()
if ("symptoms" %in% which_to_run)  run_symptoms()
if ("disorders" %in% which_to_run) run_disorders()
stamp("Done.")
