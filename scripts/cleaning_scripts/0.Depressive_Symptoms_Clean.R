...
# CLEAN MDD FILES
sumstats <- sumstats %>% filter(cleaning_script == 'DepressiveSyptoms_Clean')
...
sumstats_cleaned$Depressive_Sx <- sumstats_cleaned$Depressive_Sx %>%
  filter(str_detect(variant_id, "^rs")) %>%  # Keep valid rsIDs
  rename(
    ID = variant_id,
    CHROM = chromosome,
    A1 = effect_allele,
