...
for (list in trait_lists) {
  ...
  rds_file_path <- file.path(folderpaths$gsem.mvLD, rds_file_name)
    ...
    ld_result_hm3 <- ldsc(
      ...
      sample.prev = as.numeric(sumstats[sumstats$code %in% selectedCodes, ]$gsem_samplePrevalence),
      population.prev = as.numeric(sumstats[sumstats$code %in% selectedCodes, ]$populationPrevalence),
