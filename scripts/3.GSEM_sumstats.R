...
disorders <- sumstats %>% 
  filter(Disorder_RGs == 1)
disorders <- disorders$code


# Step 2: Create trait list vector
trait_lists <- list(
  all,
  symptoms,
  disorders
...
variant_lists<- lapply(variant_lists, function(x) gsub("~/GENIE_GSEM/", "", x))

sumstats$cleanedpath <- paste0("data/gwas_sumstats/", sumstats$cleaned_file)
...
# Only run if the file doesn't already exist
      ...
      gsem_sumstats <- sumstats(
        ...
        betas = NULL,
        N = as.vector(as.numeric(sumstats_filtered$gsem_n)) #  previously used N = sumstatsToMunge$n_total - from tutorial: " If the summary statistics file includes a sample size column, then the user can also list NA if they wish to use the SNP-specific sample sizes to perform the rescaling. However ,we note again that this should be the sum of effective sample sizes for dichotomous traits. "
