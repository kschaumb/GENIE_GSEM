...
ldsc_path <- file.path(foldererpaths$gsem.mvLD,
  "gsem", "mvld",
  paste0("mvLD_", paste(disorders, collapse = "_"), ".Rds")
)
...
clean_names <- trait_order |>
  ...
  gsub("_", " ", x = _) %>% 
  recode("AN" = "Anorexia Nervosa",
         "ANX" = "Anxiety Disorders",
         "BE noAN" = "Binge Eating (no AN)",
...
ggplot() +
  ...
  theme_minimal() +
  labs(title = "(a) Genetic Correlation Matrix - Key Traits and Disorders",
       x = "Trait", y = "Trait") +
...
ggsave("gsem/gsem_output/Genetic_Correlation_Matrix_Disorders.png",
